...
function makeVirtualizer(t) {
  ...
  const options = {
    ...
    estimateSize: () => 100,
    getScrollElement: () => el,
    observeElementRect: (instance, cb) => { cb({ width: 100, height: 400 }); return () => {}; },
    observeElementOffset: (instance, cb) => { t.offsetCb = cb; return () => {}; },
    scrollToFn: (offset) => { writes.push(offset); el.scrollTop = offset; },
    initialOffset: 0,
  };
