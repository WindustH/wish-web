...
function setup() {
  ...
  const document = Object.assign(new EventTarget(), {
    ...
    documentElement: {
      ...
      style: { setProperty: (key, value) => widths.set(key, value) },
    },
  ...
  const result = new Function('cfg', 'tryPlatform', 'document', 'window', 'exports',
    ...
    cfg, () => ({ get: key => saved.get(key), set: (key, value) => saved.set(key, value) }), document, window, {});
  const start = () => result.onListResizePointerDown({ button: 0, currentTarget: target,
    pointerId: 4, clientX: 400, preventDefault() {} });
  const event = (kind, values = {}) => window.dispatchEvent(Object.assign(new Event(kind), { pointerId: 4, clientX: 440, buttons: 1, ...values }));
