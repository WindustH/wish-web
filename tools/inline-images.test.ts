...
import { splitAttachmentText, attachmentsForMessage } from '../src/core/attachmentPlaceholders.ts';
import { orderedUserContent, userBubbles } from '../src/core/api/userContent.ts';
...
test('deleted markers omit images while legacy images and files remain attached', () => {
  const legacy: { kind: string; placeholder?: string } = { kind: 'image' }, file: typeof legacy = { kind: 'file' };
...
test('persisted display order produces the requested six independent bubbles', () => {
  ...
  assert.equal((bubbles[2] as { text: string }).text, 'text2 text3');
  assert.equal((bubbles[3] as { blob_id?: string }).blob_id, 'session/2');
});

 test('SHA-256 matches native hashing including padding boundaries and binary images', async () => {
  const { createHash, randomBytes } = await import('node:crypto');
  const { attachmentDigest, attachmentDigestFallback } = await import('../src/core/attachmentDigest.ts');
