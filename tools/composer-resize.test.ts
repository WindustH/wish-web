...
function setup() {
  ...
  const document = Object.assign(new EventTarget(), {
    hidden: false,
    documentElement: { classList: { add: (value: string) => classes.add(value), remove: (value: string) => classes.delete(value) } },
  });
  ...
  const { useComposerDrag } = new Function('onScopeDispose', 'cfg', 'document', 'window',
    ...
    (fn: () => void) => { dispose = fn; }, { composer: { resizeStep: 20 } }, document, window);
  const committed: number[] = [];
  const sizing = { min: () => 100, max: () => 500, change() {}, commit: (value: number) => committed.push(value) };
