...
function NewSessionForm({ onClose }) {
  ...
  const { providers, provider, setProvider, models, loading, loadErr, retry } = useProviderModels();
  ...
  return html`<${Modal} title=${i18n.t('new.title')} onClose=${onClose} dismissable=${!busy}
    footer=${html`
      <${Button} disabled=${busy} onClick=${onClose}>${i18n.t('new.cancel')}<//>
    ...
    <div class="field">
      ...
      <input class="input" placeholder=${i18n.t('new.namePlaceholder')} value=${name} disabled=${busy}
        onInput=${(e) => setName(e.target.value)} />
    </div>
    <div class="field">
      <label>${i18n.t('new.provider')}</label>
      <select class="select" value=${provider} disabled=${busy || !providers} onChange=${(e) => { setProvider(e.target.value); setModel(''); }}>
    ...
    <div class="field">
      ...
      <select class="select" value=${model} onChange=${(e) => setModel(e.target.value)} disabled=${busy || !models?.length}>
        <option value="">—</option>
    ...
    <div class="field">
      ...
      <textarea class="textarea" rows="3" value=${agentCustom} disabled=${busy}
