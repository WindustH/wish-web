// Session browser: search box + new button on top, list below.
    ...
    <div class="sessions-pane">
      ...
      <div class="sl-list" role="list">
        ...
        ${!loading && !err && items.length === 0 && html`
          <div class="sl-empty">${query ? i18n.t('sessions.emptySearch') : i18n.t('sessions.empty')}</div>`}
        ${items.map((s) => html`<${SessionRow} key=${s.id} s=${s} active=${s.id === activeId} />`)}
