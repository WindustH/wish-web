...
与生产环境的区别：开发模式下没有 Service Worker，所以安装和更新流程只能在构建产物上测试（`./pnpmw build && node serve.mjs`）。主机名和来源检查属于 `serve.mjs`，开发服务器上也没有。
...
| `./pnpmw test` | 单元测试：`node --test tools/*.test.mjs` |
| `./pnpmw selftest:api [url]` | 对运行中的服务做只读的 API 检查，默认 `http://127.0.0.1:8790` |
| `./pnpmw start` | `node serve.mjs` |

## 测试

- **单元测试**（`./pnpmw test`，安装依赖后在仓库根目录运行）：`tools/*.test.mjs`，基于 `node:test`。它们在不启动浏览器的情况下测试各类逻辑：API 数据转换和模型校验、聊天历史和消息队列、粘贴文本和附件占位符、配置合并、提供商就绪判断、列表和输入框的尺寸调整，以及已打补丁依赖的回归测试。
...
- **文案。** 所有可见文字都要有中英两个版本。多处共用的文字写成 `src/core/i18n/zh.js` 和 `en.js` 中的键（两个文件的键必须一致），通过 `i18n.t('key')` 使用；只在一处出现的文字可以直接写成 `tr('中文', 'English')`。
...
- **参数与平台。** 数值写在 `src/core/config.js` 中，不要写死在组件里。存储、通知、文件、剪贴板和屏幕唤醒一律通过 `src/platform/` 的 `platform()` 访问，不要直接调用浏览器 API。
...
| 补丁 | 修复的问题 | 对应测试 |
| --- | --- | --- |
| `patches/reka-ui@2.10.4.patch` | 滚动锁定不再修改页面的 `pointer-events`（交给可关闭层处理）；即使退出动画没有报告结束，关闭的对话框也会被卸载 | `tools/scroll-lock.test.mjs`、`tools/presence-lifecycle.test.mjs` |
| `patches/@tanstack__virtual-core@3.17.9.patch` | 在后台保持挂载的虚拟列表出现跳动和卡死：过期的滚动校正会被丢弃，而不是重放 | `tools/virtual-clamped-adjustment.test.mjs` |
