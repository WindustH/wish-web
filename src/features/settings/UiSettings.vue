...
<template>
  <div class="ui-settings">
    ...
    <div class="cfg-layout">
      <nav class="cfg-sections" :aria-label="tr('配置分类', 'Configuration sections')"><button v-for="item in sections" :key="item.id" type="button" :class="{ active: section === item.id }" :aria-current="section === item.id ? 'page' : undefined" @click="section = item.id">{{ tr(item.zh, item.en) }}</button></nav>
      <div class="cfg-content">
    <p v-if="problem" class="cfg-notice cfg-error" role="alert">{{ problem }}</p><p v-if="notice" class="cfg-notice" role="status">{{ notice }}</p>
    <section v-show="section === 'appearance'" class="cfg-preferences"><h2>{{ tr('外观与语言', 'Appearance and language') }}</h2>
    ...
    <section v-show="section === 'input'" class="cfg-preferences"><h2>{{ tr('输入与通知', 'Input and notifications') }}</h2>
    ...
    <section v-show="section === 'local'" class="cfg-preferences"><h2>{{ tr('应用与本地数据', 'Application and local data') }}</h2>
      ...
      <div class="setting-row"><div><span>{{ tr('连接诊断', 'Connection diagnostics') }}</span><p class="cfg-hint">{{ tr('检查当前浏览器与后端的连接状态。', 'Check connectivity between this browser and the backend.') }}</p></div><RouterLink class="btn" to="/selftest"><Activity :size="16" />{{ tr('检查连接', 'Check connection') }}</RouterLink></div>
