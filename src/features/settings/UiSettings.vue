    ...
    <SettingsSections prefix="ui" :sections="sections">
      ...
      <template #default="{ section }">
        <template v-if="section === 'appearance'">
          <div class="setting-row"><div><label for="ui-theme">{{ tr('主题', 'Theme') }}</label><SettingHint id="ui-theme-hint" :text="tr('选择浅色、深色，或随操作系统自动切换。', 'Choose light, dark, or follow your operating system.')" /></div><SelectField segmented id="ui-theme" aria-describedby="ui-theme-hint" :model-value="mode" :options="[{ value: 'auto', icon: 'monitor', label: tr('跟随系统', 'Follow system') }, { value: 'light', icon: 'sun', label: tr('浅色', 'Light') }, { value: 'dark', icon: 'moon', label: tr('深色', 'Dark') }]" @update:model-value="theme.setMode" /></div>
          <div class="setting-row"><div><label for="ui-language">{{ tr('界面语言', 'Language') }}</label><SettingHint id="ui-language-hint" :text="tr('只更改按钮和说明的语言，不翻译对话内容。', 'Changes interface labels and help; conversations are not translated.')" /></div><SelectField id="ui-language" aria-describedby="ui-language-hint" :model-value="locale" :options="[{ value: 'zh', label: '中文' }, { value: 'en', label: 'English' }]" @update:model-value="i18n.setLocale" /></div>
        </template>
        <template v-if="section === 'input'">
          <div v-if="desktop" class="setting-row"><div><label for="send-on-enter">{{ tr('按 Enter 发送消息', 'Send with Enter') }}</label><SettingHint id="send-on-enter-hint" :text="sendOnEnter ? tr('按 Shift + Enter 换行。', 'Press Shift + Enter for a new line.') : tr('按 Enter 换行，按 Ctrl / ⌘ + Enter 发送。', 'Press Enter for a new line; Ctrl / ⌘ + Enter to send.')" /></div><SwitchRoot id="send-on-enter" aria-describedby="send-on-enter-hint" :model-value="sendOnEnter" class="cfg-switch" @update:model-value="prefs.setSendOnEnter"><SwitchThumb class="cfg-switch-thumb" /></SwitchRoot></div>
          <div v-else class="setting-row"><div><label for="keep-awake">{{ tr('运行时保持屏幕唤醒', 'Keep screen awake while running') }}</label><SettingHint id="keep-awake-hint" :text="tr('仅在浏览器支持屏幕唤醒时生效。', 'Available when the browser supports screen wake lock.')" /></div><SwitchRoot id="keep-awake" aria-describedby="keep-awake-hint" :model-value="keepAwake" class="cfg-switch" @update:model-value="prefs.setKeepAwake"><SwitchThumb class="cfg-switch-thumb" /></SwitchRoot></div>
          <div class="setting-row"><div><label for="failure-notifications">{{ tr('运行失败时通知我', 'Notify me when a run fails') }}</label><SettingHint id="failure-notifications-hint" :text="tr('页面在后台时显示系统通知。', 'Show system notifications while the page is in the background.')" /></div><SwitchRoot id="failure-notifications" aria-describedby="failure-notifications-hint" :model-value="notifyOnFailure" class="cfg-switch" @update:model-value="setNotifications"><SwitchThumb class="cfg-switch-thumb" /></SwitchRoot></div>
        </template>
        <template v-if="section === 'local'">
          <div class="setting-row"><div><span>{{ tr('安装 Wish', 'Install Wish') }}</span><SettingHint :text="app.isStandalone() ? tr('正在使用独立应用窗口。', 'Running in an application window.') : tr('从桌面或主屏幕直接打开 Wish。', 'Open Wish directly from your desktop or home screen.')" /></div><button id="ui-install" class="btn" @click="install"><Download :size="16" />{{ tr('安装', 'Install') }}</button></div>
          <div class="setting-row"><div><span>{{ tr('连接诊断', 'Connection diagnostics') }}</span><SettingHint :text="tr('检查当前浏览器与后端的连接状态。', 'Check connectivity between this browser and the backend.')" /></div><RouterLink id="ui-diagnostics" class="btn" to="/selftest"><Activity :size="16" />{{ tr('检查连接', 'Check connection') }}</RouterLink></div>
          <div class="setting-row"><div><span>{{ tr('本地设置', 'Local preferences') }}</span><SettingHint :text="tr('清除浏览器保存的偏好和输入草稿。后端会话不受影响。', 'Clear preferences and drafts stored in this browser. Server sessions remain available.')" /></div><button id="ui-clear-local" class="btn danger" @click="clearOpen = true">{{ tr('清除本地数据', 'Clear local data') }}</button></div>
        </template>
      </template>
