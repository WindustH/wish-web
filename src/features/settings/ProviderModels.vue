...
function edit(id='',seed:Record<string,any>={}){originalId.value=id;modelId.value=id;draft.value=JSON.parse(JSON.stringify(id?props.value.models[id]:seed));effortsInput.value=draft.value.reasoning_efforts?Object.keys(draft.value.reasoning_efforts).join(', '):'';error.value='';advanced.value=JSON.stringify(draft.value,null,2);advancedChanged.value=false;editSource.value=JSON.stringify([modelId.value,draft.value]);editing.value=true;}
...
<template>
  <div class="models-editor">
    ...
    <div v-for="(model,name) in value.models" :key="name" class="model-row">
      <button class="model-edit" :aria-label="tr('编辑模型 ','Edit model ')+name" @click="edit(String(name))"><strong>{{model.display_name||name}}</strong><small>{{name}} · {{tr('上下文','Context')}} {{model.context_window_tokens??'—'}} · {{tr('输出','Output')}} {{model.max_output_tokens??'—'}}<template v-if="model.reasoning_efforts"> · {{tr('思考','Reasoning')}} {{Object.keys(model.reasoning_efforts).join('/')}}</template></small></button>
