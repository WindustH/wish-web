...
function fill(v: string, attachmentsToFill?: any[]) {
  ...
  if (attachmentsToFill?.length) void refillAttachments(attachmentsToFill);
}
...
function onEditorInput(event: Event) {
  ...
  else setTextOwned(value);
}
function onEditorPaste(event: ClipboardEvent) {
  if (!btwMode.value) onPaste(event);
}
function submitBtw() {
...
async function submit() {
  ...
  const sent = attachments.value;
  submitting.value = true;
  try {
    const receipt = await (props.sendMessage
      ? props.sendMessage(payload, sent)
      : chat.send(payload, sent));
    if (!receipt) return;
    if (currentSid.value !== owner) {
      chat.setDraft('', owner);
      saveAttachmentDrafts(
        owner,
        attachmentDraftsFor<Attachment>(owner).filter((i) => !sent.includes(i)),
      );
      revokeAll(sent);
      return;
    }
  ...
  <div ref="composerEl" class="composer" :class="[mobile ? 'mobile' : 'desktop', { 'composer-start': start }]"
    ...
    <div class="composer-editor">
      <textarea ref="ta" :rows="cfg.composer.mobileMinRows"
        :placeholder="btwMode ? (i18n.locale.value==='zh'?'向 BTW 提问…':'Ask BTW…') : running ? i18n.t('chat.placeholderRunning') : i18n.t('chat.placeholder')"
        :aria-label="btwMode ? (i18n.locale.value==='zh'?'向 BTW 提问':'Ask BTW') : i18n.t('chat.placeholder')" :value="editorText"
        @input="onEditorInput" @keydown="onKeydown" @paste="onEditorPaste" />
      ...
      <Hint v-else-if="mobile" :text="i18n.t(running ? 'chat.stop' : 'chat.send')"><button class="send-btn" :class="{ stop: running }" :disabled="sending || (!running && !canSend)"
...
:global(.btw-bubble){position:fixed;z-index:72;display:flex;flex-direction:column;width:min(460px,calc(100vw - 16px));height:min(440px,calc(100dvh - 120px));min-height:0;overflow:hidden;background:var(--bg);border:1px solid var(--line-strong);border-radius:14px;box-shadow:var(--shadow-pop);transform-origin:bottom left}
...
:global(.btw-bubble.mobile){width:min(460px,calc(100vw - 40px));height:min(390px,calc(100dvh - 150px));border-radius:12px}
