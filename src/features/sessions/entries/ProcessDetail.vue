...
const fields=computed(()=>Object.entries(args.value).filter(([key])=>!(name.value==='shell'&&key==='command')));
const printable=(value:any)=>typeof value==='string'?value:JSON.stringify(value,null,2);
const fieldName=(key:string)=>({operation:tx('操作','Operation'),path:tx('文件','File'),edit:tx('编辑文件','Edited file'),timeout:tx('等待时间（秒）','Wait (seconds)'),execution_id:tx('进程标识','Execution ID'),query:tx('查询','Query'),limit:tx('数量上限','Limit')})[key]??key;
</script>
<template>
 <Modal ref="dialog" :open="true" compact wide :content-class="`process-detail${diffOnly||name==='view_image'?' process-detail-visual':''}`" :title="title" @close="emit('close')">
  <template #compact-heading>
   <div class="process-detail-heading"><Icon :name="isReasoning?'brain':name==='view_image'?'image':name==='shell'?'terminal':'wrench'"/><strong class="process-detail-title">{{title}}</strong><span v-if="!isReasoning&&!diffOnly" class="detail-kind">{{isResult?tx('工具结果','Tool result'):tx('工具调用','Tool call')}}</span><span class="detail-sequence">#{{isResult?step.entry.seq:step.fromSeq}}</span><CopyButton :text="diffOnly?edit?.diff??'':text"/><button class="btn ghost icon-only" :aria-label="tx('关闭','Close')" @click="dialog?.close()"><Icon name="x"/></button></div>
  </template>
  ...
  <template v-else-if="!isResult">
   <section v-if="name==='shell'&&args.command" class="tool-section"><h3>{{tx('命令','Command')}}</h3><pre class="terminal-output">{{args.command}}</pre></section>
  ...
  <template v-else>
   ...
   <template v-if="name==='shell'&&output?.process">
   ...
   <template v-else-if="name==='search_history'&&Array.isArray(output?.items)">
...
@media(max-width:599px){.modal-card.process-detail{max-height:84dvh}.process-detail-heading{gap:6px}.process-detail-heading .detail-sequence{display:none}.process-detail-title{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.process-detail-heading .detail-kind{flex-shrink:0}.process-detail-heading .copy-btn{margin-left:auto}.process-detail-heading>.btn{flex-shrink:0}.tool-fields>div{grid-template-columns:1fr;gap:3px}.process-detail .modal-body{padding:16px}.terminal-output{font-size:12px}}
</style>
