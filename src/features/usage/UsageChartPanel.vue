...
const series = computed(() => models.value.filter(model => !hidden.value.has(model.key)).map(model => ({ key: model.key, label: model.label, colorIndex: models.value.indexOf(model), extendMean: metric.value === 'tps', points: metric.value === 'tps' ? model.samples.map(point => [point.at_ms, point.tps] as [number, number | null]) : model.points.map(point => [point.at, point.tokens] as [number, number | null]) })));
