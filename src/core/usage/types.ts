...
export interface UsageModel { provider: string | null; model: string | null; tokens: number; tps: number | null; samples: { at_ms: number; tps: number; duration_ms: number; output_tokens: number; attempt_id: string }[]; points: UsagePoint[] }
export interface UsageChartData { models: UsageModel[]; samples: number; truncated?: boolean; attempts: number; timezone: string }
...
export interface UsageSeriesResponse {
  ...
  sampling: { interval_ms: number; source: string; bytes_per_token: number; displayed_samples: number; sample_limit: number; truncated: boolean };
  coverage: { attempts_with_usage: number; attempts_success: number; attempts_failed: number; stream_samples: number };
  groups: { samples: { at_ms: number; tps: number; duration_ms: number; output_tokens: number; attempt_id: string }[]; provider: string | null; model: string | null; window: Omit<UsageBucket, 'start_ms'>; buckets: UsageBucket[] }[];
}
