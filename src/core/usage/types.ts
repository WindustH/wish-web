...
export interface UsageModel { provider: string | null; model: string | null; tokens: number; tps: number | null; points: UsagePoint[] }
export interface UsageChartData { models: UsageModel[]; samples: number; attempts: number; timezone: string }

export interface SeriesQuery { window: '1d' | '7d'; bucket?: '1h' | '6h'; to_ms?: number }
...
export interface UsageSeriesResponse {
  query: { from_ms: number; to_ms: number; window: '1d' | '7d'; bucket: '1h' | '6h'; bucket_ms: number };
  ...
  groups: { provider: string | null; model: string | null; window: Omit<UsageBucket, 'start_ms'>; buckets: UsageBucket[] }[];
}
