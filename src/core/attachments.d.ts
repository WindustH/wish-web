export interface AttachmentInput { kind: 'image' | 'file'; name?: string; mime: string; bytes: ArrayBuffer; localUrl?: string }
export interface PickedAttachment { kind: 'image' | 'file'; name?: string; mime: string; size: number; read(): Promise<ArrayBuffer> }
